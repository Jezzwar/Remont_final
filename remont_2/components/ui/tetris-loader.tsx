...
const TETRIS_PIECES = [
  { shape: [[1, 1, 1, 1]], color: 'bg-beige' },
  { shape: [[1, 1], [1, 1]], color: 'bg-beige' },
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-beige' },
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-beige' },
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-beige' },
  { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-beige' },
  { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-beige' },
]
...
export default function TetrisLoading({
  ...
  const createNewPiece = useCallback((): FallingPiece => {
    ...
    return { shape, color: pieceData.color, x, y: -shape.length }
  }, [rotateShape, config.gridWidth])

  const canPlacePiece = useCallback((piece: FallingPiece, nx: number, ny: number): boolean => {
  ...
  const clearFullLines = useCallback(() => {
    setGrid(prev => {
      ...
      return prev.map((row, i) =>
        lines.includes(i) ? row.map(c => ({ ...c, color: 'bg-beige-light opacity-50' })) : row
      )
    })
  }, [config.gridWidth])
      ...
      if (ts - lastUpdateRef.current >= fallSpeed) {
        ...
        if (!isClearing && !checkAndReset()) {
          setFallingPiece(prev => {
            ...
            const ny = prev.y + 1
            if (canPlacePiece(prev, prev.x, ny)) return { ...prev, y: ny }
            placePiece(prev)
        ...
        {row.map((cell, ci) => (
          <div
            ...
            className={`${config.cellSize} border border-white/[0.06] transition-all duration-100 ${
              cell.filled ? `${cell.color} scale-100` : 'bg-white/[0.03] scale-95'
            }`}
          />
        ))}
      </div>
  ...
  return (
    <div className="flex flex-col items-center">
      <div className={`border border-white/10 bg-graphite ${config.padding}`}>
        {renderGrid()}
      </div>
      {showLoadingText && (
        <p className="mt-4 text-white/40 text-xs font-heading uppercase tracking-[0.25em]">
          {loadingText}
        </p>
      )}
    </div>
